 ...
 */
const choosePlayerPrompt = async (answers = {}) => {
  if (players.length === 1) {
  ...
  return inquirer
    ...
    .then(({ player }) => players[player] || null);
};
...
const run = async () => {
  ...
  console.log(chalk.bold(`Ready ${chalk.yellow.italic(player.name)}, roll your dice! 🎲`), `\n`);
